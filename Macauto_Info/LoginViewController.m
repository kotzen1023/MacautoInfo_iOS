...
- (void)viewDidLoad {
    ...
    [_labelTitle setText:NSLocalizedString(@"MACAUTO_INFO", nil)];
...
- (NSMutableArray *) sendHttpPost {
    _loginlSInfo = [[NSMutableArray alloc] init];
    ...
    _soapMessage = [NSString stringWithFormat:@"<?xml version=\"1.0\" encoding=\"utf-8\"?>"
                    ...
                    "<soap:Body>"
                    "<Message_login xmlns=\"http://tempuri.org/\">"
                    "<user_no>%@</user_no>"
                    "<password>%@</password>"
                    "<ime_code>%@</ime_code>"
                    "</Message_login>"
    ...
    [theRequest addValue: @"http://tempuri.org/Message_login" forHTTPHeaderField:@"SOAPAction"];
  ...
  namespaceURI:(NSString *)namespaceURI qualifiedName:(NSString *)qName
{
    ...
    if ([elementName isEqualToString:@"Message_loginResponse"]) {
        _doc = false;
    } else if ([elementName isEqualToString:@"Message_loginResult"]) {
        //_MeetingList = false;
        //_checkno = false;
